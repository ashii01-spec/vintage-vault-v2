    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white border border-vintage-200 shadow-xl sm:rounded-lg p-6">
                <h2 class="text-2xl font-serif font-bold text-vintage-900 mb-6">Edit Category</h2>
                
                <form action="{{ route('admin.categories.update', $category->id) }}" method="POST">
                    @csrf
                    @method('PUT') <div class="mb-6">
                        <label class="block text-vintage-800 font-serif font-bold mb-2">Category Name</label>
                        <input type="text" name="name" value="{{ $category->name }}" class="w-full border-gray-300 rounded px-3 py-2" required>
                    </div>
                    ...
                    <div class="flex justify-end gap-4">
                        ...
                        <button type="submit" class="btn-vintage">Update Category</button>
                    </div>
                </form>
            </div>
        </div>
